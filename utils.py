...
class GameService:
    ...
    def generate_quiz_question(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            prompt = f"""Ты - мастер создания увлекательных викторин! Создай ОДИН уникальный, захватывающий вопрос викторины на тему "{industry}" на русском языке.

🎯 ТРЕБОВАНИЯ К ВОПРОСУ:
• Вопрос должен быть НЕОБЫЧНЫМ, нестандартным и вызывать интерес
• Добавь элемент загадки или исторического контекста
• Сделай вопрос личностным: "Ты знаешь, что...", "Представь себе..."
• В начале вопроса укажи отрасль в скобках: ({industry.capitalize()})
• Уровень сложности: средний-сложный (не слишком простой)

🔢 ВАРИАНТЫ ОТВЕТОВ:
• Ровно 4 варианта (1, 2, 3, 4)
• Один вариант - абсолютно правильный
• Другие варианты должны быть правдоподобными, но неправильными
• Сделай варианты разной длины и стиля
• Добавь в неправильные варианты распространенные заблуждения или близкие ответы

💡 ПОДСКАЗКА:
• Создай ПОДРОБНУЮ подсказку (3-4 предложения)
• Добавь ИНТЕРЕСНЫЕ ФАКТЫ, которые не были в вопросе
• Сделай подсказку познавательной и мотивирующей
• Можно добавить юмор или удивительные детали

🎮 СТИЛЬ:
• Вопрос должен быть увлекательным и мотивирующим
• Используй восклицательные знаки и эмодзи в мыслях
• Сделай так, чтобы игрок захотел узнать ответ!

📋 ФОРМАТ ОТВЕТА:
Вопрос: [захватывающий вопрос с отраслью в скобках]
1. [вариант ответа]
2. [вариант ответа]
3. [вариант ответа]
4. [вариант ответа]
Правильный ответ: [номер правильного варианта]
Подсказка: [интересная подробная подсказка с фактами]"""
...
• В начале вопроса укажи отрасль в скобках: ({industry.capitalize()})
• Уровень сложности: средний-сложный (не слишком простой)
• Вопрос должен глубоко погружать в тему "{industry}"
...
• Сделай варианты разной длины и стиля
• Добавь в неправильные варианты распространенные заблуждения или близкие ответы
• Все варианты должны быть связаны с темой "{industry}"
...
• Сделай подсказку познавательной и мотивирующей
• Можно добавить юмор или удивительные детали
• Свяжите подсказку с темой "{industry}"

🎮 СТИЛЬ:
• Вопрос должен быть увлекательным и мотивирующим
• Используй восклицательные знаки и эмодзи в мыслях
• Сделай так, чтобы игрок захотел узнать ответ!
• Добавь элемент интриги или удивления
...
class SmartDetector:
    """Класс для умного определения намерений пользователя в сообщениях."""

    def __init__(self):
        """Инициализирует детектор."""
        # Ключевые слова для определения математических запросов
        self.math_keywords = [
            'посчитай', 'вычисли', 'расчет', 'калькулятор', 'сколько будет',
            'реши', 'вычислить', 'математика', 'арифметика', '+', '-', '*', '/',
            'умножить', 'разделить', 'плюс', 'минус', 'равно', 'формула',
            'выражение', 'число', 'цифры', 'сумма', 'разность', 'произведение'
        ]

        # Ключевые слова для определения запросов перевода
        self.translation_keywords = [
            'переведи', 'перевод', 'translate', 'translation', 'на русский',
            'на английский', 'на испанский', 'на французский', 'на немецкий',
            'на итальянский', 'на португальский', 'на китайский', 'на японский',
            'на корейский', 'в переводе', 'означает', 'что значит'
        ]

        # Ключевые слова для определения запросов погоды
        self.weather_keywords = [
            'погода', 'weather', 'температура', 'градусы', 'дождь', 'снег',
            'облачно', 'солнечно', 'ветер', 'влажность', 'прогноз',
            'какая погода', 'что с погодой', 'погода в', 'климат'
        ]

        # Ключевые слова для определения игровых запросов
        self.game_keywords = [
            'игра', 'сыграем', 'поиграем', 'game', 'играть', 'камень ножницы бумага',
            'угадай число', 'викторина', 'кости', 'кубики', 'бросить кубик',
            'волшебный шар', 'шар судьбы', 'да или нет', 'да нет'
        ]

        # Ключевые слова для определения развлекательных запросов
        self.fun_keywords = [
            'шутка', 'joke', 'смешно', 'пошути', 'юмор', 'анекдот',
            'факт', 'интересный факт', 'цитата', 'мотивация', 'мотивируй',
            'вдохнови', 'забавный', 'развлеки', 'весело'
        ]

        # Ключевые слова для определения запросов смены персоны
        self.persona_keywords = [
            'стань', 'будь', 'превратись', 'роль', 'персона', 'характер',
            'поведение', 'стиль', 'как', 'веди себя', 'ты должен быть',
            'представь что ты', 'роль играй', 'актёр', 'актёрствуй'
        ]

    def is_math_request(self, text: str) -> bool:
        """
        Определяет, является ли сообщение математическим запросом.

        Args:
            text: Текст сообщения

        Returns:
            bool: True если это математический запрос
        """
        ...
        # Проверяем наличие математических ключевых слов
        for keyword in self.math_keywords:
            if keyword in text_lower:
                return True

        # Проверяем на наличие цифр и математических выражений
        import re
    ...
    def is_translation_request(self, text: str) -> bool:
        """
        Определяет, является ли сообщение запросом перевода.

        Args:
            text: Текст сообщения

        Returns:
            bool: True если это запрос перевода
        """
        text_lower = text.lower().strip()
    ...
    def is_weather_request(self, text: str) -> bool:
        """
        Определяет, является ли сообщение запросом погоды.

        Args:
            text: Текст сообщения

        Returns:
            bool: True если это запрос погоды
        """
        text_lower = text.lower().strip()
    ...
    def is_game_request(self, text: str) -> bool:
        """
        Определяет, является ли сообщение игровым запросом.

        Args:
            text: Текст сообщения

        Returns:
            bool: True если это игровой запрос
        """
        text_lower = text.lower().strip()
    ...
    def is_fun_request(self, text: str) -> bool:
        """
        Определяет, является ли сообщение развлекательным запросом.

        Args:
            text: Текст сообщения

        Returns:
            bool: True если это развлекательный запрос
        """
        text_lower = text.lower().strip()

        for keyword in self.fun_keywords:
            if keyword in text_lower:
                return True

        return False

    def is_persona_change_request(self, text: str) -> bool:
        """
        Определяет, является ли сообщение запросом смены персоны.

        Args:
            text: Текст сообщения

        Returns:
            bool: True если это запрос смены персоны
        """
        text_lower = text.lower().strip()
...
class AdvancedTools:
    """Класс для продвинутых инструментов и функций."""

    def __init__(self):
        """Инициализирует продвинутые инструменты."""
        pass

    # Здесь можно добавить различные продвинутые функции в будущем
    # Пока оставляем как placeholder
